...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart2, REQUEST, sizeof(REQUEST));
  /* USER CODE END 2 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
  if (REQUEST[0] == 98 && REQUEST[1] == 0 && REQUEST[2] == 0 && REQUEST[3] == 226){
	  uint8_t RESPONSE[18] = {"\x10TMBelka1234567EP\x90"};
	  HAL_UART_Transmit(huart, RESPONSE, sizeof(RESPONSE), HAL_MAX_DELAY);
	  HAL_UART_Receive_IT(huart, REQUEST, sizeof(REQUEST));
  }
  if (REQUEST[0] == 34 && REQUEST[3] == 162){
  	  uint8_t RESPONSE[4] = {2, REQUEST[1], '\x44', '\x82'};
  	  HAL_UART_Transmit(huart, RESPONSE, sizeof(RESPONSE), HAL_MAX_DELAY);
  	  HAL_UART_Receive_IT(huart, REQUEST, sizeof(REQUEST));
  }
  if (REQUEST[0] == 66 && REQUEST[3] == 194){
  	  uint8_t RESPONSE[2] = {'\x00', '\x80'};
  	  HAL_UART_Transmit(huart, RESPONSE, sizeof(RESPONSE), HAL_MAX_DELAY);
  	  HAL_UART_Receive_IT(huart, REQUEST, sizeof(REQUEST));
  }
}
